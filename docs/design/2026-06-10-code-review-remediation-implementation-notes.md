...
### Design decisions
- **A single shared helper, not per-file conventions.** `tests/common/mod.rs::otto_cmd(home: &Path)` pins `OTTO_HOME` and removes `OTTO_DB_PATH` on every command it builds. Every `tests/*.rs` file that spawns the binary now either calls it directly or (`dynamic_choices_test.rs`, `foreach_command_test.rs`) reuses its own fixture's `TempDir` as the isolated home through it, so the isolation logic lives in one place rather than being re-derived per file.
- **`OTTO_HOME` plus `env_remove("OTTO_DB_PATH")`, not `HOME` alone.** `resolve_otto_home()` (`src/executor/layout.rs`) checks `OTTO_HOME` before `HOME`, and `default_db_path()` (`src/executor/state/db.rs`) checks `OTTO_DB_PATH` before that. `tests/history_table_alignment_test.rs` previously set only `HOME`, which is silently defeated by a developer's own exported `OTTO_HOME`/`OTTO_DB_PATH`; the fix pins both.
- **`tests/db_isolation_test.rs` pins the helper itself, not just its callers.** One test proves the helper isolates the DB; a second proves it wins even when `OTTO_DB_PATH` is already exported in the parent process (`unsafe { std::env::set_var(...) }`, `#[serial]`-guarded), matching the exact scenario a developer's real shell creates.
- **The retention parity flake fixed at the fixture, with the boundary itself pinned separately.** `ports::db::tests::memory_and_sqlite_stores_agree_about_retention` placed a fixture run exactly on the `keep_failed_days` cutoff it exercises, then called `SystemTime::now()` twice (once per backend) to check it — any wall-clock tick between the two calls could flip that run's expiry differently per backend. The fixture (`src/ports/db_tests.rs`) now keeps a multi-day margin around every cutoff; the exact-boundary tie-break (`timestamp == cutoff` is kept, not expired) is pinned deterministically with an injected `now` in `src/executor/state/retention_tests.rs::test_keep_failed_days_exact_boundary_is_kept_not_expired`.
- **The ETXTBSY flake in `run_version`** was already fixed in `b744b04` before this phase started; audited every other `Command::new` site in `src/` for the same write-then-immediately-exec pattern and found none — the race is specific to `upgrade.rs`'s install/verify flow and its own tests.

### Deviations
- **Found and fixed a live instance of the exact bug this phase targets, not a hypothetical one.** `tests/cli_surface_test.rs` (3 tests) and `tests/builtin_commands_test.rs` (4 tests) spawned the binary with no `OTTO_HOME` while every sibling test in the same files set it. A full `cargo test --workspace --all-features` run with both env vars unset hit this gap once in roughly a dozen tries during this phase's own verification, writing project rows named after their `tempdir()` fixtures straight into the real `~/.otto/otto.db`. These two files predate this phase and were not in the design doc's own list of gaps; found by auditing every test function's body for a spawn call site missing `OTTO_HOME`, not by the doc. Fixed the same way as their sibling tests; re-verified clean across 10+ subsequent full-suite runs, including cold-rebuild runs that reproduce the original conditions.
- **`tests/flag_integration_test.rs` mutates the process-wide cwd (`std::env::set_current_dir`) without a `#[serial]` guard on at least one path** (investigated as a candidate root cause of the leak above, ruled out: the file never spawns the binary or touches `StateManager`, so it can't itself write to the real DB). Left unfixed: it's a real global-mutable-state smell but a different bug class than this phase's DB-isolation ask, and touching it risks changing behavior this phase wasn't asked to verify.
- **Bullets 523-530 (Parser/Scheduler/cfg/State-ports/Action-output/Upgrade/TUI/Converter test gaps) and the CI coverage-gate bullet were read in full but not implemented.** The phase as written asks for roughly fifty individual new tests across eight subsystems; the actual assignment for this pass scoped effort to the home-isolation gap (the newest, self-measured bullet, added after the author's own DB audit) and the two named flakes. Left `[ ]` unchecked in the design doc rather than marked done; the doc's own "Deferred, not fabricated" note records this explicitly.
- **The Observed-to-test inventory table covers the Acceptance Criteria section's seven `Observed on main` lines, not every `Observed` line in every phase.** Phases 0-10 already ship each fix with its own regression test per this phase's opening note; auditing all of them (dozens of `Observed` lines across ten phases) was judged out of proportion to what the inventory table is for (falsifiable proof that the headline bugs are covered), given the scope decision above.

### Tradeoffs
- **`ports::db_tests.rs`'s fixture ages chosen explicitly (an array of tuples) rather than reusing the original formulaic loop with a skip.** A formula that special-cased "skip day 45" would have been less legible than just listing the ten (age, status) pairs directly; the explicit list also documents, in the data itself, which ages are deliberately on which side of the two cutoffs the test exercises.
- **`examples_integration_test.rs`'s `otto_cmd()` now returns `(TempDir, Command)` instead of bare `Command`.** The alternative (a `OnceLock`/shared scratch dir across all example tests) would couple unrelated tests through one shared mutable home; a fresh throwaway `TempDir` per call costs a syscall but keeps every test's isolation independent, matching the pattern used everywhere else in this phase.
- **`tests/makefile_converter_test.rs`'s `run_convert` sets `OTTO_HOME`/`env_remove("OTTO_DB_PATH")` directly on `std::process::Command` rather than routing through `common::otto_cmd`.** `assert_cmd::Command` doesn't expose raw `stdin`/`stdout`/`stderr` piping (needed here to feed a Makefile on stdin and capture both streams), so this one call site duplicates the helper's two env calls instead of using it. Documented inline in the test so the duplication doesn't drift silently.

### Open questions
- **Should the ~139 residual `.tmp*`-named project rows currently in `~/.otto/otto.db` be deleted?** Some predate this session (historical residue); some were added by this phase's own reproduction attempts while diagnosing the leak above (each subsequently investigated, none left the DB in a broken state — `otto ci` and the full suite are green). The author cleaned exactly this kind of residue by hand on 2026-08-29 (1040 rows down to 224). Left untouched this time rather than unilaterally deleted, since it's the user's real, persistent data; a full backup was taken before this phase's work began (`~/.otto/otto.db.pre-phase11-backup-<epoch>`).
- **`tests/flag_integration_test.rs`'s unguarded `std::env::set_current_dir`** (see Deviations) is a real test-isolation smell worth its own fix, but is a different bug class than this phase's DB-isolation ask; flagging rather than fixing under this phase's mandate.
- **The nine test-gap bullets (523-530) and the CI coverage-gate bullet remain open**, per the Deviations note above — confirm whether they get their own follow-up phase/doc or are dropped from this plan's scope now that the plan's headline (Phase 0-10 gates plus DB isolation) is closed.

## Phase 11 follow-up: the remaining test-gap bullets, plus the CI coverage gate

Continuation of Phase 11 on top of `cf768eb`. The prior pass scoped its effort to the home-isolation gap and the two named flakes, leaving the eight Parser/Scheduler/cfg/State-ports/Action-output/Upgrade/TUI/Converter bullets and the CI coverage-gate bullet unchecked; this pass closes all of them.

### Design decisions
- **Audit before writing, every sub-item.** For each of the ~35 named sub-items across the eight bullets, checked for an existing test by name before writing anything new. Roughly two-thirds were already closed, several by fixes that landed after the doc's 2026-08-29 re-verification (e.g. the `=`-escape-hatch naming and the help-path ottofile bug, both still tagged "STAYS: explicitly NOT taken" in the doc's prose but demonstrably fixed in `indices()`/`ottofile_value_from_args`). Citing an existing test rather than duplicating coverage.
- **`ActiveTasks::reap_unreported` and the completion-channel tests go through `super::*`, not a mock.** Both are private to `executor::scheduler`; `scheduler_tests_a`/`_b` are child modules (`#[path] mod tests_a`), which see private items of their parent, so both test the real internal struct directly rather than through a test double.
- **The completion-channel backpressure bullet ended up as three tests, not one, because the first one didn't discriminate the regression it was meant to catch.** Live spot-check: swapped the real `tx.send(report).await` in `scheduler/task_execution.rs` for `tx.try_send(report)` (a genuine drop-on-full bug) and reran the first test (40 tasks against a scheduler) - it stayed green, because these tasks finish fast enough that the channel never actually filled in that run. Added a bare-channel test proving the general backpressure property, and a source-text check (`include_str!` + `!source.contains("try_send")`) as the actual mechanical guard - confirmed, by the same live edit, to be the only one of the three that goes red.
- **The two-process concurrency test uses real, separate `StateManager`/SQLite connections on separate OS threads against one db file, not a mock.** This is the closest a single-process test can get to genuine multi-process contention: each thread opens its own connection (as a real second `otto` process would), exercising WAL mode and the 5s busy timeout for real rather than asserting they exist.
- **The RealFs/MemFs equivalence suite runs the identical script against both backends and diffs the recorded outcomes, rather than asserting each backend's behavior separately.** This is what caught two divergences neither backend's own existing tests (28 of them) could have caught in isolation.
- **CI coverage-gate wiring verified by forcing failure, not just by reading the YAML.** Set `--fail-under` to an unreachable `99.9`, ran the real chain (`otto cov` standalone, then the full `otto ci`), and confirmed the process exit code was `1` — caught and corrected an initial false read where a `| tail` pipe was masking the real exit code. Then re-ran at the real floor (85) to confirm the pass path too.

### Deviations
- **The coverage floor (85) was measured locally, not on a CI runner.** The design doc's mechanism explicitly says "Phase 11 sets X to the first green-on-a-runner measurement"; this pass could not trigger or observe an actual GitHub Actions run, so 85 is a conservative floor under the local 90.35% measurement, not the runner measurement the doc asked for. Recorded as a real gap in Open Questions, not silently treated as satisfying the instruction.
- **The "revert the fix, confirm red" half of the Success Criteria was spot-checked, not done exhaustively.** Done live for the newer-schema-refusal migration test and all three completion-channel tests (the latter caught a real gap: the first of the three did not go red under the exact regression it was meant to catch, which is why there are three); not repeated for all ~35 assertions this pass touches. Recorded as unverified-at-scale in the doc's own Success Criteria line rather than claimed complete.
- **Two RealFs/MemFs divergences were found and documented, not fixed.** `MemFs::write` auto-creates a missing parent directory where `RealFs::write` errors; `MemFs::exists` never consults its own `symlinks` map. Both are real gaps in the test double's fidelity to the real filesystem. Fixing them is a `MemFs` behavior change (production code, not test-writing), and no caller in `src/` or `tests/` currently depends on either divergent behavior, so it was left as a documented difference rather than changed under a test-gap-closure mandate.
- **The fs-root workspace-root case (an ottofile literally at `/`) was not integration-tested.** Writing to the real filesystem root of the machine running the suite is not something a test should do, isolated or not. The recursion-termination logic (`find_ottofile`'s "reached the root, stop" branch) is exercised anyway, every time a search from a tempdir fails to find an ottofile and walks all the way up to the real `/`.

### Tradeoffs
- **`otto-<version>-<timestamp>.backup` adversarial-filename test asserts the actual (correct, non-panicking) behavior rather than the behavior initially assumed.** A non-numeric timestamp segment (`otto-1.2.3-notanumber.backup`) is accepted with `timestamp = 0` (via `.unwrap_or(0)`), not rejected outright — the test was corrected to assert that once the real output was observed, rather than asserting the assumption and leaving a wrong pin in place.
- **`BackupInfo` gained `#[derive(Debug)]`** so the adversarial-filenames test could assert-and-print on failure. A one-line, harmless production change made to enable the test, not a behavior change.
- **Wired `cov` into `ci`'s `before:` list, not `cov-report` directly**, since `cov-report` has no dependency edge of its own onto `cov` (it is only ever triggered by `cov`'s `after:`); depending on `cov-report` directly from `ci` would run it with no coverage data to read.
