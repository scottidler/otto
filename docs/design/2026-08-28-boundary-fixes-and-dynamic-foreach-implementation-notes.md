...
- Positional-args sharp edge, verified on a real fixture (not asserted from the design doc's prose): ottofile with `sw` (positional param `svc`) and `web` (unrelated task) in the same file. `otto sw web` mis-partitions: `sw` runs with `svc` unbound and fails (`script.sh: line 17: svc: unbound variable`), and `web` runs as its own separate task — reproduced exactly as the doc predicted. `otto sw someservice` (no collision) correctly binds `svc=someservice`. Both transcripts are in the new doc page verbatim.
