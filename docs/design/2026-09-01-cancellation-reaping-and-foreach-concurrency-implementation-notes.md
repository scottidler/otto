...
Not a phase. This section records the remediation of the review-panel implementation audit of `440a54c..ab45e62` (Mode 2, round 1), which found no skipped Implementation Plan bullet and three shipped statements of behavior the code does not have. The user approved eight items: the three must-fixes and five cheap wins. Two commits: one for code and tests, one for docs.

**This section supersedes two earlier claims in this file, which are append-only and so are corrected here rather than edited in place:**

- **Line 95 (Phase 3, Deviations) says `warn_on_tty_with_foreach_jobs` "says so once per run". It fires once per ITEM, and the warning is now deleted.** `tty:` is inherited by every subtask (`src/cfg/task.rs:963-967` says so outright) and `foreach_jobs` is stamped on every item at expansion (`src/cli/parser/discovery.rs:400-404`), so the filter matched N times. Measured by the audit on a five-item group: five lines, naming `tails:a`..`tails:e` - items, not the `tails` the ottofile wrote. It was also a `log::warn!`, and otto's logging goes to the XDG log file rather than stderr (`src/main.rs:91`), so the count was academic: nobody saw any of them.
- **Line 166 (Phase 5, Design decisions) says the docs page documents the resolution as "tty wins, warned once per run".** Both halves are now false. `tty: true` with `foreach.jobs` is a load error, and `docs/commands/buffered-foreach.md`'s `tty: true` subsection describes that.

That also closes the Phase 3 open question at line 123 ("Should `tty: true` combined with `foreach.jobs` be a load error?"). The answer is yes, and it landed exactly where that note predicted it belonged: one validator beside `validate_foreach_jobs`.

### Design decisions

- **`validate_foreach_jobs_tty` is its own function, not a second check inside `validate_foreach_jobs`** — `src/cli/parser/config.rs` — one function per rejected shape is the file's existing convention (`validate_foreach_sources`, `validate_required_params`, `validate_foreach_buffer`, `validate_foreach_jobs`), and it keeps each call site in `load_config` readable as one sentence about one shape.
- **The `--Serial` rejection lives at the foreach expansion seam, not in a validator** — `src/cli/parser/foreach.rs`, beside `run_serial` — `--Serial` is a per-task CLI partition, so it does not exist at config-load time, where every sibling rejection lives. Expansion is the first place where both halves of the contradiction are in hand (the flag and the `foreach:` block), and it is before anything executes, which is what the other rejections actually promise. `run_serial` moved above the `jobs.insert` so the check runs before the key is recorded rather than after.
- **The `admission_for` tty arm stays, with a doc comment that says why it is unreachable from an ottofile.** The loader now rejects the combination, so no loaded `Task` can take it. Deleting the arm would make the function partial for a `Task` built by hand (which is how the unit tests build them) and would leave the terminal-safety choice implicit. The unit test `tty_wins_over_a_group_concurrency_override` was kept and reframed for the same reason: it pins the classification for the hand-built case and no longer cites a function that no longer exists.
- **The new criterion (f) test is an integration test with barriers, not a unit test** — `tests/foreach_jobs_concurrency_test.rs::a_queued_exempt_item_does_not_park_the_launch_loop` — the property is about `execute_task`'s wiring, and `execute_task` is private to `src/executor/scheduler/task_execution.rs`, so a sibling unit test cannot call it. The observable chosen is order-independent: a task that can only be launched by a pass that already popped the queued item. Whatever order the ready queue happens to be in, a parked loop never reaches it.

### Deviations

- **None from the approved eight.** All eight landed. The audit's four "defer" items (`CANCEL_GRACE` in the doc, Phase 4's upgrade hint on a mis-nested `jobs:`, scoping `Status: Implemented` past the otto-dev row, the version bump) were not approved and were not touched.

### Tradeoffs

- **Rejecting `tty` + `jobs` vs keeping tty-wins.** Rejecting breaks any ottofile that set both and relied on being tolerated. Nothing in this repo or `otto-dev` does, the key is one release old, and the alternative was a file that answers the same question two ways eleven lines apart (`validate_foreach_buffer` rejects `buffer` + `tty` for the identical reason). Fail closed at load.
- **`--Serial` + `jobs` rejected vs `jobs` silently ignored when the run is serial.** Ignoring is the smaller blast radius and is what the code did by accident. Rejecting matches the `parallel: false` answer, and the residue of ignoring was not nothing: the serial group's in-flight item stayed exempt from `capped_len()`, so capped tasks ran past `-j` beside it.
- **Amending the design doc vs recording the gaps only here.** Amending, for the three claims that were false: a criterion nobody can meet as written, and a Risks row contradicting the Architecture paragraph two hundred lines above it, are doc defects, not implementation failures, and leaving them for a reader to rediscover is what the audit just cost. Every amendment is marked in place and its evidence is below.

### The doc amendments, with evidence

- **Acceptance Criterion 1, "no descendant of any task body" -> "of a non-`tty` task body"** (design doc, Acceptance Criteria). Doc defect, not a failing implementation: the criterion contradicts Phase 1's own criterion (c), which asserts that cancelling a `tty: true` task must not signal otto itself. Evidence in code: `src/executor/scheduler/task_execution.rs:243-249` registers the tty child with `own_group: false`, and `signal_child` (`src/executor/scheduler.rs:612`) branches on it to send `kill` rather than `killpg` - otto is in that process group, so a group signal would hit otto. The hole was disclosed in these notes twice (Phase 1 Tradeoffs and Open questions) and nowhere in the design doc. Narrowed to the carve-out and no further: every non-tty body's descendants still have to die.
- **Risks table row, "blocks every later task, including a `tty:` one" -> "holds up a later `tty:` task"** (design doc, Risks and Mitigations). False as written. `may_admit(Admission::Capped, _)` returns `true` unconditionally (`src/executor/scheduler.rs`), and `capped_len()` counts only `is_capped()` classes, so an ordinary later task is admitted beside an exempt group. Pinned in-tree by this repo's own fixture comment at `tests/foreach_jobs_concurrency_test.rs:413-416` ("The gate is an ordinary capped task and the group is exempt, so it can run beside them even at `-j 1`"), and that gate does run in `a_tty_task_becoming_ready_during_an_exempt_group_waits_for_it`. The Architecture paragraph (the same doc, `Consequence, stated rather than discovered later`) was always correct and narrow; the Risks row was the overclaim, and `docs/commands/buffered-foreach.md` copied the row rather than the paragraph. Both are corrected; the paragraph was left alone except for the starvation shape below.
- **Head-of-line skipping documented** (design doc Architecture, and `docs/commands/buffered-foreach.md`). Behavior kept, docs fixed - both audit seats said so independently. A tty task the loop cannot admit is pushed back to the head of the ready queue and the pass continues (`src/executor/scheduler.rs`, `deferred_by_admission`), so later exempt items start ahead of it and it can wait indefinitely. It was reasoned out in these notes (Phase 3 Tradeoffs) and stated in neither doc.
- **API Design table specifies outcome, not mechanism** (design doc, API Design). The table's rows were all true and each is pinned by a test (`tests/foreach_jobs_test.rs`, four tests); only the framing clause "in the shape `validate_foreach_buffer` already uses" was wrong, since three of the four rejections come free from the `ForeachJobs` deserializer and `deny_unknown_fields`. Reworded, and the two new rejections added as rows.
- **`buffered-foreach.md`'s "This is not silent - otto logs a warning once per run"** rewritten to describe the load error. Doubly false (see the two superseded claims above) and moot once the warning was deleted.

### Verification

- `cargo test --workspace --all-features`: 44 `test result: ok` lines, 0 failed, across the lib tests and every integration binary.
- Both new rejections run against the built binary, not only through tests: `otto -o otto.yml tails --Serial` -> `Task 'tails': --Serial cannot be combined with foreach.jobs; serial means one item at a time, so a concurrency override is incoherent`, rc=1; the same file without the flag -> `[tails] finished successfully`, rc=0. `tty: true` + `jobs: all` -> `Task 'tails': foreach.jobs cannot be combined with tty; a tty task owns the terminal exclusively, so it runs exclusively and a per-group concurrency override cannot be honored`, rc=1.
- **Break-the-code for the new criterion (f) test, run both ways.** Hoisting the acquire out of the spawned body (`let hoisted = semaphore.clone().acquire_many_owned(permits).await?;` above `active.spawn`, the body taking `hoisted`): `a_queued_exempt_item_does_not_park_the_launch_loop` FAILED - `wait_for(the task behind the queued item starts) timed out`, otto's output empty - while `spawn_counts_a_task_in_flight_before_its_body_acquires_a_permit` stayed `ok`. That is precisely the gap the audit named, now closed. Patch reverted; `git status` clean before the commit.
