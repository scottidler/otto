...
## Post-audit cheap-wins

Three cheap-wins from the round-4 implementation audit (`/tmp/review-panel/V0HS3VJH/synthesis-r4.md`), landed after Status: Implemented, in one commit.

### Design decisions
- **EdgeSpec inventoried.** `EdgeSpec` (`src/cfg/edge.rs`) enforces the same "unknown field" contract as the six Architecture-table structs via a hand-written `visit_map` rather than `#[serde(deny_unknown_fields)]`; it had no reference-doc rows and no drift-test coverage, so a third field would have tripped nothing. Added a new "Edge object (`EdgeSpec`) — 2 keys" section to `docs/commands/ottofile-reference.md` (`tasks.<name>.after[].task`/`.when`), added `EdgeSpec` to the drift test's exhaustive destructuring (all 4 fields, since 2 are on-disk keys and 2 — `from_sugar`, `is_injected_sugar` — are bookkeeping) and to the `expectations` table, moved the total 44 -> 46. Verified `expected_keys_from_deny_unknown_fields::<EdgeSpec>()` produces the identical "unknown field `x`, expected `task` or `when`" shape the derive macro produces for a two-field struct (no location suffix, same phrasing) — no special-casing needed.
- **PROOF.md unbroken.** `examples/interactive-demo/PROOF.md` still instructed the reader to write `interactive: true`, a key Phase 2 already sed'd out of the sibling `otto.yml` (`tty:` superseded it in v1.3.0). Fixed the three reader-instructional occurrences (the "Tasks Loaded with" header, the "YAML parsing" proof bullet, and the "ADD THIS LINE" worked example under "The Original Use Case") to `tty: true`.
- **Drift test made path-aware.** `reference_doc_mentions_key` (src/cfg/task.rs) matched only the trailing dot-segment of a backtick span, so it was blind to level: deleting the root `tasks` row, the `tasks.<name>.envs` row, or a `help` row all passed vacuously (some OTHER row ending in the same segment — `otto.tasks`, `otto.envs`, `...params.<title>.help` — silently vouched for the deleted one). Replaced with `reference_doc_mentions_key_at`, an exact full-path match, plus one path-builder function per struct (`root_path`, `otto_path`, `retention_path`, `task_path`, `foreach_path`, `param_path`, `edge_path`) mirroring the doc's own level headings. Full paths were chosen to exactly match the doc's own existing conventions per level (bare `otto`/`tasks` at root, `otto.<key>`, `otto.retention.<key>`, `tasks.<name>.<key>`, `tasks.<name>.foreach.<key>`, the doc's own `...params.<title>.<key>` ellipsis convention, and `tasks.<name>.after[].<key>` for edges) rather than reconstructing a generic path from struct nesting, since the doc doesn't follow one uniform convention across levels.

### Deviations
- None from the audit's three cheap-win specs. `EdgeSpec`'s destructuring binds all 4 struct fields (not just the 2 on-disk keys) so the compile-time trigger still fires on `from_sugar`/`is_injected_sugar` changes too — a superset of what the audit asked for the same reason the other six structs' destructuring binds every field.

### Tradeoffs
- Per-struct path-builder functions (7 small `fn(&str) -> String`) vs. a single generic "compute path from struct name" function. Chosen because the reference doc does not use one uniform nesting convention across all seven levels (root keys are bare, `ParamSpec` rows use a literal `...` ellipsis instead of the full `tasks.<name>.params.<title>` chain) — a generic function would have had to special-case exactly the same set of exceptions, so naming them explicitly per struct is no more code and is easier to audit against the doc by eye.

### Open questions
- None.

### Verification
1. **EdgeSpec inventory.** Reference doc has rows for `tasks.<name>.after[].task`/`.when`; drift test destructures `EdgeSpec` and includes it in `expectations`; total asserted at 46. `cargo test --lib cfg::task::foreach_tests::ottofile_reference_key_inventory_is_exhaustive` -> `1 passed; 0 failed`. Proof it bites: added a scratch `pub scratch_drift_field: bool` to `EdgeSpec` -> `cargo test --lib ...` failed to COMPILE, `error[E0027]: pattern does not mention field \`scratch_drift_field\` --> src/cfg/task.rs` (plus 2 more `E0063`s at `EdgeSpec`'s own two real construction sites in `src/cli/parser.rs`). Field reverted; `git diff --stat -- src/cfg/edge.rs` empty; test green again.
2. **PROOF.md unbroken.** Built the exact task PROOF.md now instructs (`tty: true` in place of `interactive: true`) in a scratch dir and ran it: `tty task ran` / `[shell] finished successfully`, exit 0. Confirmed the OLD instruction still errors on today's schema (for contrast): `tasks.shell: unknown field \`interactive\`, expected one of \`help\`, \`after\`, \`before\`, \`input\`, \`output\`, \`envs\`, \`params\`, \`bash\`, \`python\`, \`action\`, \`foreach\`, \`on-failure\`, \`tty\` at line 4 column 5`, exit 1. Re-ran the repo-wide markdown sweep for `interactive:`, `defaults:`, `otto.task:`, task-level `show:`/`verbosity:`: PROOF.md's remaining `interactive:` hits are the new historical-staleness disclaimer note and the untouched, admittedly-stale internal code excerpts (sections 2-5, see Open questions below) — none instruct the reader to write the key into their own ottofile anymore.
3. **Drift test path-aware.** Deleted the root `tasks` row -> **FAILS**: `` ConfigSpec's on-disk key `tasks` (expected doc path `tasks`) is not mentioned... `` (previously passed vacuously). Restored. Deleted the `tasks.<name>.envs` row -> **FAILS**: `` TaskSpecHelper's on-disk key `envs` (expected doc path `tasks.<name>.envs`) ... `` (previously passed vacuously). Restored. Deleted the `tasks.<name>.help` row -> **FAILS**: `` TaskSpecHelper's on-disk key `help` (expected doc path `tasks.<name>.help`) ... `` (previously passed vacuously). Restored. Non-regression: deleted the `tasks.<name>.tty` row -> still **FAILS** (as before the fix): `` TaskSpecHelper's on-disk key `tty` (expected doc path `tasks.<name>.tty`) ... ``. Restored; `diff` against the pre-edit backup empty.
4. `otto ci`: **green**, `[ci] ✅ All CI checks passed!`. Full suite `cargo test --workspace --all-features --no-fail-fast -- --test-threads=4` -> **748 passed, 0 failed** across 27 binaries, matching the round-4 auditor's own baseline exactly; the `OTTO_HOME` test-isolation flake did not reproduce.
5. `git status --short -- src/ docs/ examples/ tests/` before commit staging: only `docs/commands/ottofile-reference.md`, `examples/interactive-demo/PROOF.md`, `src/cfg/task.rs` modified (plus this notes file). No scratch mutations left behind.

### Open questions
- `examples/interactive-demo/PROOF.md` sections 2-5 quote a PTY-based implementation (`InteractivePty`, `PtyIoProxy`, `execute_interactive_task`, `interactive_semaphore`, a DB `interactive` column) that no longer exists anywhere in `src/` — confirmed via `grep -rn` returning zero hits for all four names. This predates the `interactive:` -> `tty: true` rename entirely; today's `tty: true` is a much simpler `Stdio::inherit()` (`src/executor/scheduler.rs`), no PTY. Fixing only the reader-facing config-key instructions (this cheap-win's actual scope) left that deeper staleness in place; added an inline disclaimer note rather than silently leaving it or silently rewriting the whole narrative. Flagging for a follow-up: either delete `PROOF.md` as a dead artifact of a scrapped design, or rewrite sections 2-5 to describe the current `tty:` mechanism.

### Note superseding Phase 2's stale open item (`:62`)
Per this notes file's append-only rule, `:62`'s "`examples/ex1/otto.yml` bullet in `docs/flag-support.md` points at a path that does not exist" is stale, not rewritten in place. Phase 5 fixed it in `b0ad2bb` (`git show b0ad2bb -- docs/flag-support.md` shows the `examples/ex1/otto.yml` reference removed), but the Phase 5 notes section never recorded that closure. Recording it here: **closed, by `b0ad2bb`, during Phase 5** — not by this post-audit pass.
