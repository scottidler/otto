...
- **2026-09-03 amendment: `yaml_serde`, not `serde_yaml_ng`.** The choice above was revisited, not reversed in reasoning: `serde_yaml_ng` (single-maintainer `github.com/acatton/serde-yaml-ng`, MIT) had had no release since ~May 2024 by the time this was checked; `yaml_serde` (`github.com/yaml/yaml-serde`, the YAML organization's own repo, dual MIT/Apache-2.0 like the original `serde_yaml`, `0.10.7`) is API/byte-compatible with `serde_yaml 0.9.34` and verified via `cargo info`. Same four symbols, same seam, same strict-schema suite as the compatibility check; see Resolved Decisions and the implementation notes' post-phase amendment section.
...
- **2026-09-03: revisited, `yaml_serde` replaces `serde_yaml_ng`.** The 2026-09-02 call above stands as the reasoning for leaving `serde_yaml` at the time; it is not being rewritten. New evidence triggered a second look: `serde_yaml_ng` is a single-maintainer fork (`github.com/acatton/serde-yaml-ng`, MIT) at `0.10.0` with no release since roughly May 2024. `yaml_serde` (`github.com/yaml/yaml-serde`, the YAML organization's own repo, dual MIT/Apache-2.0 matching the original `serde_yaml`, `0.10.7`, `rust-version: 1.82`, comfortably under this repo's pinned 1.96.0 toolchain) is API- and byte-compatible with `serde_yaml 0.9.34`, verified against the existing strict-schema suite (`unknown field`, `line N column M` pins unchanged) and the `--tasks --format json` regression guard. `serde-saphyr` remains out of scope for the same reason as before: a different error model and no dynamic `Value` type.
...
### Dependencies
- Removed from `[dependencies]`: `regex` (to dev), `expanduser`, `dirs`, `num_cpus`, `serde_yaml`. Added: `serde_yaml_ng`, later replaced in place by `yaml_serde` (2026-09-03 amendment, Resolved Decisions). Net: four fewer crates in the normal tree, five fewer transitive (`dirs 1.0.5`, `redox_users 0.3.5`, `winapi`, `pwd`, `lazy_static`).
