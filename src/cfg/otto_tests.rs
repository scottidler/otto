...
fn test_retention_spec_deserialize_empty() {
    let yaml = "{}";
    let spec: RetentionSpec = yaml_serde::from_str(yaml).unwrap();
...
retention:
  ...
  keep_last: 5
"#;
    let spec: OttoSpec = yaml_serde::from_str(yaml).unwrap();
...
fn wrap_unknown_field_error_names_the_key_and_the_upgrade_fix() {
    ...
    let yaml = "otto:\n  not-a-real-key: foo\ntasks:\n  up:\n    bash: echo hi\n";
    let raw_err = yaml_serde::from_str::<ConfigSpec>(yaml).unwrap_err();
...
fn test_retention_spec_roundtrip() {
    ...
    let deserialized: RetentionSpec = yaml_serde::from_str(&yaml).unwrap();
...
mod jobs_range_tests {
    ...
    fn jobs_zero_is_rejected_at_config_load() {
        let err = yaml_serde::from_str::<ConfigSpec>("otto:\n  jobs: 0\ntasks:\n  build:\n    bash: true\n")
    ...
    fn a_positive_jobs_value_still_loads() {
        for n in [1usize, 2, 64] {
            ...
            let spec: ConfigSpec = yaml_serde::from_str(&yaml).expect("positive jobs must load");
