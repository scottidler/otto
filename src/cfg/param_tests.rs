...
fn test_boolean_flag_detection_true_default() {
    ...
    let task_spec: TaskSpec = yaml_serde::from_str(yaml).unwrap();
...
fn a_minimal_param_serializes_with_no_null_valued_keys() {
    ...
    let yaml = "params:\n  filename:\n    help: Input file\nbash: echo hi\n";
    let task_spec: TaskSpec = yaml_serde::from_str(yaml).unwrap();
    let emitted = yaml_serde::to_string(&task_spec).unwrap();
...
fn nargs_valid_range_still_parses() {
    let nargs: Nargs = yaml_serde::from_str("\"2:5\"").unwrap();
...
fn test_nargs_roundtrip_all_variants() {
    ...
    for nargs in cases {
        let yaml = yaml_serde::to_string(&nargs).unwrap();
        let parsed: Nargs = yaml_serde::from_str(&yaml).unwrap_or_else(|e| panic!("failed to parse {yaml:?}: {e}"));
...
fn choices_command_serializes_back_to_the_kebab_case_key() {
    ...
    let emitted = yaml_serde::to_string(&spec).unwrap();
...
fn choices_and_choices_command_together_is_a_loud_config_error() {
    use crate::cfg::task::TaskSpec;
    let err = yaml_serde::from_str::<TaskSpec>(
        r#"
            params:
              -s|--svc:
                choices: [alpha, beta]
...
fn a_bare_nargs_count_means_exactly_that_many() {
    let nargs: Nargs = yaml_serde::from_str("\"3\"").unwrap();
    assert_eq!(nargs, Nargs::Range(3, 3));
    assert_eq!(yaml_serde::to_string(&nargs).unwrap().trim(), "'3'");
...
fn a_zero_bounded_span_is_still_rejected() {
    let err = yaml_serde::from_str::<Nargs>("\"0:5\"").unwrap_err().to_string();
